...
class InCityJobsDiagnostics:
    ...
    def get_secure_connection():
        ...
        try:
            # First attempts to read from local deployment variables
            db_url = st.secrets["postgres"]["url"]
        except Exception:
            try:
                # Falls back to alternative global variables
                db_url = st.secrets["connection_string"]
            except Exception:
                # Direct fail-safe fallback targeting your active Neon database
